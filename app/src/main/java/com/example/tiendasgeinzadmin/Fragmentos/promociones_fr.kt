...
import com.geinzTienda.tiendasgeinzadmin.databinding.FragmentPromocionesFrBinding
import com.google.firebase.auth.FirebaseAuth
import com.google.firebase.firestore.FirebaseFirestore
import com.google.firebase.firestore.SetOptions
import com.google.firebase.storage.FirebaseStorage

class promociones_fr : Fragment() {

    private lateinit var firebaseAuth: FirebaseAuth
    private lateinit var mcontex: Context
    private lateinit var binding: FragmentPromocionesFrBinding
    private var imgNoticia: Uri? = null
    private var reserva:Boolean?=null
    private var adquirir:Boolean?=null
    ...
    override fun onCreateView(
        ...
        binding = FragmentPromocionesFrBinding.inflate(layoutInflater, container, false)
        return binding.root
    }
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        binding.crear.setOnClickListener {
            agregarNoticia(notificar)
        }

        binding.usarMetodoPago.setOnClickListener {
            metodos_pago_dialog.mostrarDialogoGuardarMetodoPago(mcontex,binding.yape,binding.plin,binding.efectivo,yapeCheked,plinChaked,efectivoCheked)
        }
        ...
        val grupoAdquirir = binding.grupoAdquirir
        grupoAdquirir.setOnCheckedChangeListener { grupo, checkId ->
            adquirir = when (checkId) {
                R.id.adquirir_si -> true
                R.id.adquirir_no ->false
                else->null
            }
        }
    ...
    private fun agregarNoticia(notificacion:Boolean) {
        val db = FirebaseFirestore.getInstance().collection("Tiendas")
            .document(firebaseAuth.uid.toString()).collection("promociones")
        val newArticuloRef = db.document()
        val idPromo = newArticuloRef.id
        val hashMap = hashMapOf<String, Any>(
            "id" to idPromo,
            "titulo" to binding.TituloPromo.text.toString(),
            "Contenido" to binding.descripcionPromo.text.toString(),
            "imagenUrl" to "",
            "precio" to binding.precioArticuloEditext.text.toString(),
            "stok" to binding.stokED.text.toString(),
            "lugaresDisponibles" to "barranca",
            "vencimiento" to binding.vencimientoED.text.toString(),
            "reserva" to (reserva?:false),
            "adquirir" to (adquirir?:false),
            "yape" to yapeCheked,
            "plin" to plinChaked,
            "efectivo" to efectivoCheked

            )
        newArticuloRef.set(hashMap).addOnSuccessListener { res ->
            Toast.makeText(mcontex, "Promocion subida exitosamente", Toast.LENGTH_SHORT).show()
            val ruta = "Tiendas/${firebaseAuth.uid.toString()}/Promociones/$idPromo.jpg"
            imgNoticia?.let { SubirImgFirestore("imagenUrl", it,idPromo,ruta) }
            if (notificacion) {
            ...
            .addOnFailureListener {
                Toast.makeText(mcontex, "Error al subir la noticia", Toast.LENGTH_SHORT).show()
            }
    }

    private fun SubirImgFirestore(hasmapDb: String, uri: Uri, id: String, ruta: String) {
        val db = FirebaseFirestore.getInstance().collection("Tiendas")
            .document(firebaseAuth.uid.toString()).collection("promociones").document(id)
